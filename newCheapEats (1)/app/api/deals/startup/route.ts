...
export async function GET() {
  try {
    ...
    // Check if we have recent deals (generated today)
    const todaysDeals = existingDeals.filter(deal => deal.dateAdded === today)
    
    if (todaysDeals.length > 50) {
      console.log(`✅ Found ${todaysDeals.length} deals for today, no generation needed`)
      return NextResponse.json({
        status: 'deals_current',
        message: `${todaysDeals.length} deals already exist for today`,
